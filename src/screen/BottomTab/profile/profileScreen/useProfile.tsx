...
import { getUserProfile, ImagePickerResult, uploadAvatarImage } from '@redux/Api/authService';
...
const useProfile = () => {
  ...
  const fetchUserProfile = useCallback(async () => {
    ...
    try {
      ...
      // Also apply cache busting during regular fetches to keep everything in sync
      if (res.avatar) {
  ...
  const uploadProfileAvatar = useCallback(
    async (image: ImagePickerResult, resetImageCallback?: () => void) => {
      ...
      setUploading(true);
      try {
        await uploadAvatarImage(token, image);
