...
import { useSelector } from 'react-redux';
import { RootState } from '@redux/store';
import EmptyListCustom from '../emptyList/EmptyListCustom';
  ...
  return (
    <View style={{ height: type === "Friend" ? Dimensions.get('window').height * 0.5 : Dimensions.get('window').height * 0.8 }}>
      ...
      <FlatList
        data={filteredUsers}
        ListEmptyComponent={() => (
          <EmptyListCustom message = {t("emptyState.nousers")} />
        )}
        keyExtractor={(item) => item?.id}
        renderItem={renderItem}
        keyboardShouldPersistTaps="handled"
        style={[styles.listContainer, styles.scrollContainer]}
        // contentContainerStyle={[styles.listContent, type === 'createGroup' && styles.listContentCreateGroup]}
        initialNumToRender={18}
        removeClippedSubviews
        ...
        refreshing={isRefreshing}
        onRefresh={() => fetchFriendsPage({ page: 1, query: search, replace: true })}
        ListFooterComponent={() => isLoadingMore ? <ActivityIndicator style={{ margin: 12 }} color={Color.primary} /> :
